...
// New UI Element Selector (optional, could draw directly on canvas too)
const nanobotCountElement = document.getElementById('nanobotCount'); // Example ID, add this to HTML if using
...
const BASE_BULLET_DAMAGE = 10; const BULLET_PIERCE_MAX = 1; // Max enemies a piercing bullet can hit

const ENEMY_CIRCLE_COLOR = '#F44336'; const ENEMY_SQUARE_COLOR = '#FF9800'; const ENEMY_TRIANGLE_COLOR = '#9C27B0';
const ENEMY_SHOOTER_COLOR = '#673AB7'; // New enemy color
const ENEMY_SPLITTER_COLOR = '#00BCD4'; // New enemy color
const BASE_ENEMY_RADIUS = 15; const BASE_ENEMY_HEALTH = 30; const BASE_ENEMY_SPEED = 40;
const ENEMY_SPEED_RANDOMNESS = 15;
const ENEMY_SHOOT_COOLDOWN = 2500; // Milliseconds between shots for shooter enemy
const ENEMY_BULLET_SPEED = 200; const ENEMY_BULLET_RADIUS = 4; const ENEMY_BULLET_DAMAGE = 8; const ENEMY_BULLET_COLOR = '#FF5722';
...
const POWERUP_HEALTH_COLOR = '#4CAF50'; const POWERUP_MAGNET_COLOR = '#E91E63'; const POWERUP_FREEZE_COLOR = '#00E5FF'; // For future
const AUTO_AIM_POWERUP_COLOR = '#00BCD4'; const AUTO_AIM_DROP_CHANCE = 0.03;
const HEALTH_PACK_AMOUNT = 25; // Health restored by pack
const MAGNET_DURATION = 5000; // Duration of coin magnet effect
...
const NANOBOT_SPEEDUP_COST = 180; // Cost for faster infection upgrade
const SHIELD_EXPLODE_COST = 200; // Cost for shield explosion upgrade
const BULLET_PIERCE_COST = 250; // Cost for piercing upgrade
...
const NANOBOT_INFECTION_SPEED_MULTIPLIER = 1.5; // 50% faster infection per level
...
const DAMAGE_NUMBER_LIFESPAN = 800; const DAMAGE_NUMBER_SPEED = -30; // Pixels per second upwards
...
const WAVE_CLEAR_MSG_DURATION = 1500; // How long "Wave Cleared" message shows

// Chaves LocalStorage
const STORAGE_KEY_PREFIX = 'circleShooterNanoV3_'; // Increment version
...
const STORAGE_KEY_NANOBOT_INFECT_LEVEL = STORAGE_KEY_PREFIX + 'nanobotInfectLevel'; // New upgrade key
const STORAGE_KEY_SHIELD_EXPLODE_UNLOCKED = STORAGE_KEY_PREFIX + 'shieldExplodeUnlocked'; // New upgrade key
const STORAGE_KEY_BULLET_PIERCE_UNLOCKED = STORAGE_KEY_PREFIX + 'bulletPierceUnlocked'; // New upgrade key
...
let damageNumbers = []; // New array for floating damage text
...
let waveTimer = 0; let waveClearMessageTimer = 0; // Timer for "Wave Cleared" message
...
const player = {
    ...
    nanobotInfectLevel: 0, // Track level for faster infection
    shieldExplodeUnlocked: false, // Track if shield explosion is unlocked
    bulletPierceUnlocked: false, // Track if piercing is unlocked
    shieldActive: false, shieldState: 'inactive', shieldTimer: 0,
    lastBulletTime: 0,
    activePowerups: {}, // Stores endTime for active powerups like { type: endTime }
};

// --- Upgrade Definitions --- (Added new upgrades)
const upgrades = [
...
function lerpColor(colorA, colorB, t) {
    ...
    try {
        ...
        const rgb = rgbA.map((start, i) => start + (rgbB[i] - start) * t); // Lerp directly
        return rgbToHex(rgb);
