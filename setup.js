...
const STORAGE_KEY_DAMAGE_LEVEL = STORAGE_KEY_PREFIX + 'damageLevel'; // Explicitly store level too
...
const player = {
    ...
    shieldUnlocked: false, weaponLevel: 0, damageMultiplier: 1, damageLevel: 0, // Store damage level explicitly
...
// --- Upgrade Definitions ---
const upgrades = [
    { id: 'shield', name: 'Unlock Shield', description: 'Deploy a temporary shield (Space / S).', cost: SHIELD_UNLOCK_COST, levelKey: null, isPurchased: () => player.shieldUnlocked, action: () => { if (!player.shieldUnlocked) { player.shieldUnlocked = true; updateUpgradeUI(); updateShopOverlayUI(); saveGameData(); } } },
    ...
    { id: 'damage', name: 'Damage Up', description: `Increase bullet damage by ${DAMAGE_INCREASE_AMOUNT * 100}%.`, cost: DAMAGE_UP_COST, levelKey: 'damageLevel', // Use damageLevel key
      maxLevel: 10, action: (level) => { player.damageLevel = level; player.damageMultiplier = 1 + (level * DAMAGE_INCREASE_AMOUNT); } // Update both level and multiplier
    },
...
function lerpColor(colorA, colorB, t) {
    const hexToRgb = hex => hex.match(/\w\w/g)?.map(x => parseInt(x, 16)); // Optional chaining
    ...
    try {
        ...
        console.error("Color lerp error:", e, colorA, colorB);
        return '#FFFFFF';
    }
}
...
function loadGameData() {
    try {
        ...
        // IMPORTANT: Recalculate multiplier based on loaded level
        player.damageMultiplier = 1 + (player.damageLevel * DAMAGE_INCREASE_AMOUNT);

        updateCashDisplay();
    } catch (e) {
        console.error("Error loading game data:", e);
        // Optionally reset to defaults here if loading fails
    }
}
function saveGameData() {
    try {
        ...
        // Don't need to save multiplier, it's derived from level
        // localStorage.setItem(STORAGE_KEY_DAMAGE_MULTIPLIER, player.damageMultiplier.toString()); // Removed
...
// --- Configuração Inicial Canvas --- (Resize handled in main.js init)
function resizeCanvas() {
    ...
    if (gameState === 'start' || gameState === 'gameOver' || !player.x || !player.y) {
        player.x = canvas?.width / 2 || 300; // Default fallback size
