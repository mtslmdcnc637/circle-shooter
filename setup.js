...
const inGameShopBtn = document.getElementById('inGameShopBtn'); // Certifique-se que este seletor existe
const shopCloseButtonX = document.getElementById('shopCloseButtonX'); // Seletor para o novo botão X
...
enemyImage.onload = () => {
    ...
    // Se o jogo já estiver rodando, talvez precise redesenhar?
    // Ou apenas garantir que o desenho use a flag enemyImageLoaded.
};
enemyImage.onerror = () => {
    console.error("Failed to load enemy SVG image.");
    enemyImageLoaded = false; // Marcar como não carregada
};
enemyImage.src = 'https://i.postimg.cc/y8q1wLbq/gene-structure-svgrepo-com.png'; // URL da imagem


// --- Constantes --- (O resto das constantes permanece igual)
...
// Cores dos inimigos podem ser usadas para health bars ou outlines agora
...
const POWERUP_DROP_CHANCE = 0.10; const POWERUP_DURATION = 7000; const POWERUP_LIFESPAN = 15000; // Powerups disappear after 15s
...
// --- Estado do Jogo --- (Declarado UMA VEZ com let)
let cash = 0; let health = 100; let gameState = 'start'; // <<< Estado inicial é 'start'
...
let animationFrameId = null; let shakeOffsetX = 0; shakeOffsetY = 0;
...
// --- Upgrade Definitions --- (Sem alterações aqui, mantenha a sua versão)
const upgrades = [
    ...
    { id: 'nanobot', name: 'Deploy Nanobot', description: `Launch a bot to convert an enemy (${NANO_BOT_DEPLOY_COST} Cash). Hotkey: B`, cost: NANO_BOT_DEPLOY_COST, levelKey: null, isPurchasable: true, action: () => deployNanoBot() } // A ação aqui provavelmente deveria ser apenas um marcador, a compra real acontece em outro lugar
];
...
function loadGameData() { try { cash = parseInt(localStorage.getItem(STORAGE_KEY_CASH) || '0'); player.shieldUnlocked = localStorage.getItem(STORAGE_KEY_SHIELD_UNLOCKED) === 'true'; player.weaponLevel = parseInt(localStorage.getItem(STORAGE_KEY_WEAPON_LEVEL) || '0'); player.damageLevel = parseInt(localStorage.getItem(STORAGE_KEY_DAMAGE_LEVEL) || '0'); player.fireRateLevel = parseInt(localStorage.getItem(STORAGE_KEY_FIRE_RATE_LEVEL) || '0'); player.bulletSpeedLevel = parseInt(localStorage.getItem(STORAGE_KEY_BULLET_SPEED_LEVEL) || '0'); player.shieldDurationLevel = parseInt(localStorage.getItem(STORAGE_KEY_SHIELD_DURATION_LEVEL) || '0'); player.shieldCooldownLevel = parseInt(localStorage.getItem(STORAGE_KEY_SHIELD_COOLDOWN_LEVEL) || '0'); player.nanobotInfectLevel = parseInt(localStorage.getItem(STORAGE_KEY_NANOBOT_INFECT_LEVEL) || '0'); player.shieldExplodeUnlocked = localStorage.getItem(STORAGE_KEY_SHIELD_EXPLODE_UNLOCKED) === 'true'; player.bulletPierceUnlocked = localStorage.getItem(STORAGE_KEY_BULLET_PIERCE_UNLOCKED) === 'true'; player.damageMultiplier = 1 + (player.damageLevel * DAMAGE_INCREASE_AMOUNT); updateCashDisplay(); } catch (e) { console.error("Error loading game data:", e); } }
function saveGameData() { try { localStorage.setItem(STORAGE_KEY_CASH, cash.toString()); localStorage.setItem(STORAGE_KEY_SHIELD_UNLOCKED, player.shieldUnlocked.toString()); localStorage.setItem(STORAGE_KEY_WEAPON_LEVEL, player.weaponLevel.toString()); localStorage.setItem(STORAGE_KEY_DAMAGE_LEVEL, player.damageLevel.toString()); localStorage.setItem(STORAGE_KEY_FIRE_RATE_LEVEL, player.fireRateLevel.toString()); localStorage.setItem(STORAGE_KEY_BULLET_SPEED_LEVEL, player.bulletSpeedLevel.toString()); localStorage.setItem(STORAGE_KEY_SHIELD_DURATION_LEVEL, player.shieldDurationLevel.toString()); localStorage.setItem(STORAGE_KEY_SHIELD_COOLDOWN_LEVEL, player.shieldCooldownLevel.toString()); localStorage.setItem(STORAGE_KEY_NANOBOT_INFECT_LEVEL, player.nanobotInfectLevel.toString()); localStorage.setItem(STORAGE_KEY_SHIELD_EXPLODE_UNLOCKED, player.shieldExplodeUnlocked.toString()); localStorage.setItem(STORAGE_KEY_BULLET_PIERCE_UNLOCKED, player.bulletPierceUnlocked.toString()); } catch (e) { console.error("Error saving game data:", e); } }

// --- Configuração Inicial Canvas & Input --- (Logs de debug removidos para clareza)
function resizeCanvas() {
    ...
    if (gameState === 'start' || gameState === 'gameOver' || !player.x || !player.y) { player.x = canvas?.width / 2 || 300; player.y = canvas?.height / 2 || 200; }
    aimX = canvas?.width / 2 || 300; aimY = canvas?.height / 2 || 200;
}
function updateAimPosition(event) {
    if (!canvas) return; const rect = canvas.getBoundingClientRect(); let clientX, clientY;
    if (event.pointerType === "touch") { clientX = event.clientX; clientY = event.clientY; } // Use clientX/Y for pointer events
    else if (event.clientX !== undefined) { clientX = event.clientX; clientY = event.clientY; } else return;
